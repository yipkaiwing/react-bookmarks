...
const Filtern = ({ itemList }) => {
  ...
  return (
    <div className="w-full sm:w-3/4 md:w-2/3 lg:w-1/2 xl:w-2/5 mx-auto text-md">
      ...
      <div>
        <ol className="space-y-1 text-green-500 font-bold">
          {filteredList?.map((item) => (
            <li
              className="m-4 sm:m-6 md:m-8 lg:m-10 list-disc list-inside
              flex-col space-y-1
              bg-slate-200 roundedp p-2 flex justify-center items-center"
              key={item.id}
            >
